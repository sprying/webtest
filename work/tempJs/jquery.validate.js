...
$.extend($.validator, {
	...
	defaults: {
		...
		unhighlight: function(element, errorClass, validClass) {
			...
			$(element).parent().removeClass(errorClass);//wxy 因为radio与checkbox的存在，需要将外围TD里的errorclass一并删除，因为在这两个组件中出错时添加ERRORCLASS时没啥效果，所以就在最外围的TD里一起加CSS
		}
	},
	...
	prototype: {
		...
		defaultShowErrors: function() {
			...
			this.hideErrors();
			this.addWrapper( this.toShow ).show();//wxy
		},
	...
	attributeRules: function(element) {
        ...
        delete rules.maxlength;//为了去掉maxlength的校验提示20120831 by csd
		return rules;
	},
...
myValidate.init=function(){
    ...
    if(json_objs.length>0){
      ...
      myValidate.addClass(rules,message,json_objs[0].field);
    }
}
